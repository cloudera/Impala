...
void ImpalaServer::GetInfo(TGetInfoResp& return_val,
  ...
  // The toolchain's custom thrift patch for Thrift 0.9.0 (0002-fix-thrift-union.patch)
  // requires that at least one union value is set, even if we're just returning an
  // error here. Set it to a dummy value so we don't need to fill it in all the error
  // paths below.
