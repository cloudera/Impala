...
Status SimpleScheduler::Init() {
  ...
  if (metrics_ != nullptr) {
    ...
    int num_executors = GetExecutorsConfig()->NumBackends();
    ...
    num_fragment_instances_metric_ = metrics_->AddGauge<int64_t>(
        NUM_BACKENDS_KEY, num_executors);
  }
...
void SimpleScheduler::UpdateMembership(
  ...
  // time needed to hold locks, all updates are applied to a copy of executors_config_,
...
SimpleScheduler::ExecutorsConfigPtr SimpleScheduler::GetExecutorsConfig() const {
...
void SimpleScheduler::SetExecutorsConfig(const ExecutorsConfigPtr& executor_config) {
  lock_guard<mutex> l(executors_config_lock_);
  ...
  for (const TPlanExecInfo& plan_exec_info: exec_request.plan_exec_info) {
    for (const auto& entry: plan_exec_info.per_node_scan_ranges) {
      ...
      RETURN_IF_ERROR(ComputeScanRangeAssignment(
          *executor_config, node_id, node_replica_preference, node_random_replica,
...
Status SimpleScheduler::ComputeScanRangeAssignment(
    const BackendConfig& executor_config, PlanNodeId node_id,
  ...
  // Loop over all scan ranges, select an executor for those with local impalads and collect
    ...
    if (exec_at_coord) {
      ...
      if (base_distance < TReplicaPreference::REMOTE) {
        for (const TScanRangeLocation& location: scan_range_locations.locations) {
          ...
          // Determine the adjusted memory distance to the closest executor for the replica
          // host.
          ...
          // We only need to collect executor candidates for non-remote reads, as it is the
          // nature of remote reads that there is no executor available.
      ...
      // Pick executor: host based on data location.
      ...
      executor_ip = assignment_ctx.SelectLocalExecutor(executor_candidates,
          decide_local_assignment_by_rank);
      TBackendDescriptor executor;
      assignment_ctx.SelectExecutorOnHost(*executor_ip, &executor);
      assignment_ctx.RecordScanRangeAssignment(executor, node_id, host_list,
          scan_range_locations, assignment);
    }  // End of executor host selection.
...
SimpleScheduler::AssignmentCtx::AssignmentCtx(
    const BackendConfig& executor_config,
    IntCounter* total_assignments, IntCounter* total_local_assignments)
  : executors_config_(executor_config), first_unused_executor_idx_(0),
...
const IpAddr* SimpleScheduler::AssignmentCtx::SelectLocalExecutor(
...
const IpAddr* SimpleScheduler::AssignmentCtx::SelectRemoteExecutor() {
...
bool SimpleScheduler::AssignmentCtx::HasUnusedExecutors() const {
  return first_unused_executor_idx_ < random_executor_order_.size();
}

const IpAddr* SimpleScheduler::AssignmentCtx::GetNextUnusedExecutorAndIncrement() {
  DCHECK(HasUnusedExecutors());
  const IpAddr* ip = &random_executor_order_[first_unused_executor_idx_++];
  return ip;
}

int SimpleScheduler::AssignmentCtx::GetExecutorRank(const IpAddr& ip) const {
  auto it = random_executor_rank_.find(ip);
  DCHECK(it != random_executor_rank_.end());
  return it->second;
}

void SimpleScheduler::AssignmentCtx::SelectExecutorOnHost(const IpAddr& executor_ip,
    TBackendDescriptor* executor) {
  ...
  if (executors_on_host.size() == 1) {
    *executor = *executors_on_host.begin();
    ...
    next_executor_on_host = FindOrInsert(&next_executor_per_host_, executor_ip,
        executors_on_host.begin());
    DCHECK(find(executors_on_host.begin(), executors_on_host.end(), **next_executor_on_host)
...
void SimpleScheduler::AssignmentCtx::RecordScanRangeAssignment(
  ...
  assignment_heap_.InsertOrUpdate(executor_ip, scan_range_length,
      GetExecutorRank(executor_ip));
  ...
  if (VLOG_FILE_IS_ON) {
    VLOG_FILE << "SimpleScheduler assignment to executor: " << executor.address
