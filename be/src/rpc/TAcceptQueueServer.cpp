...
void TAcceptQueueServer::ConnectionSetupHelper(int tid,
    const shared_ptr<TTransport>& item) {
...
void TAcceptQueueServer::serve() {
  ...
  const int CONNECTION_SETUP_POOL_SIZE = 1;
  ...
  ThreadPool<shared_ptr<TTransport> > connection_setup_pool("setup-server", "setup-worker",
      ...
      boost::bind<void>(&TAcceptQueueServer::ConnectionSetupHelper, this, _1, _2));
