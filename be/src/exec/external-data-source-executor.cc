...
class ExternalDataSourceExecutor::JniState {
 public:
  ...
  Status Init(Metrics* metrics) {
    ...
    num_class_cache_hits_ = metrics->CreateAndRegisterPrimitiveMetric<int64_t>(
        "external-data-source.class-cache.hits", 0);
    num_class_cache_misses_ = metrics->CreateAndRegisterPrimitiveMetric<int64_t>(
  ...
  Status UpdateClassCacheMetrics() const {
    ...
    RETURN_ERROR_IF_EXC(env);
    num_class_cache_hits_->Update(num_cache_hits);
    int64_t num_cache_misses = env->CallStaticLongMethod(executor_class_,
        get_num_cache_misses_id_);
    RETURN_ERROR_IF_EXC(env);
    num_class_cache_misses_->Update(num_cache_misses);
    return Status::OK;
  }
  ...
  Metrics::PrimitiveMetric<int64_t>* num_class_cache_hits_;
  Metrics::PrimitiveMetric<int64_t>* num_class_cache_misses_;

 private:
  JniState() : executor_class_(NULL) { }

  //DISALLOW_COPY_AND_ASSIGN(JniState);
  JniState(const JniState&);
  void operator=(const JniState&);
};

Status ExternalDataSourceExecutor::InitJNI(Metrics* metrics) {
