...
Status PartitionedHashJoinNode::ProcessBuildInput(RuntimeState* state, int level) {
  ...
  for (int i = 0; i < PARTITION_FANOUT; ++i) {
    ...
    hash_partitions_.push_back(pool_->Add(new_partition));
    RETURN_IF_ERROR(new_partition->build_rows()->Init(runtime_profile(), true));
    ...
    // optimization.
    RETURN_IF_ERROR(new_partition->probe_rows()->Init(runtime_profile(), false));
  }
