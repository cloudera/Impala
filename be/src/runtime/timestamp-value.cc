...
void TimestampValue::UtcToLocal() {
  ...
  try {
    time_t utc =
        ...
        time_of_day_.hours() * SECONDS_IN_HOUR +
        time_of_day_.minutes() * SECONDS_IN_MINUTE +
        time_of_day_.seconds();
    tm temp;
    ...
    time_of_day_ = time_duration(temp.tm_hour, temp.tm_min, temp.tm_sec,
