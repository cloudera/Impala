...
// the next heartbeat. The next heartbeat is scheduled for
// FLAGS_statestore_update_frequency_ms in the future, unless the subscriber indicated
// that it skipped processing a heartbeat, in which case the statestore will back off
...
// when it has not responded to a number of successive keep-alive heartbeats). Transience
...
class Statestore {
 ...
 private:
  ...
  // Work item passed to both kinds of subscriber heartbeat threads. First entry is the
  // *earliest* time (in microseconds since epoch) that the next heartbeat should be sent,
  // the second entry is the subscriber to send it to.
  typedef std::pair<int64_t, SubscriberId> ScheduledSubscriberUpdate;

  // The statestore has two pools of threads that send heartbeats to subscribers
  // one-by-one. One pool deals with 'keep-alive' heartbeats that update failure detection
  // state, and the other pool sends 'topic update' heartbeats which contain the
  // actual topic data that a subscriber does not yet have.
  //
  // Each heartbeat is scheduled for some time in the future and each heartbeat thread
  // will sleep until that time has passed to rate-limit heartbeats. Subscribers are
  ...
  // the queue.
  //
  // Heartbeats may be delayed for any number of reasons, including scheduler
  // interference, lock unfairness when submitting to the thread pool and head-of-line
  // blocking when threads are occupied sending heartbeats to slow subscribers
  // (subscribers are not guaranteed to be in the queue in next-heartbeat order).
  //
  // Delays for keep-alive heartbeats can result in the subscriber that is kept waiting
  // assuming that the statestore has failed. Correct configuration of keep-alive
  // heartbeat frequency and subscriber timeout is therefore very important, and depends
  // upon the cluster size. See --statestore_heartbeat_frequency_ms and
  // --statestore_subscriber_timeout_seconds.
  //
  // To mitigate this risk, 'sufficiently' many threads should be allocated to these
  // thread pool via --statestore_num_heartbeat_threads. The failure timeout for each
  // subscriber process may also be increased. In order to see timely topic updates,
  // --statestore_num_update_threads should also be set sufficiently high to make full use
  ...
  // consecutive keep-alive heartbeats, it is considered failed and a) its transient topic
  ...
  // by calling SendUpdate(). Once complete, the subscriber is re-added to the
  ...
  // SendUpdate(), and if the subscriber is judged to have failed it is removed
  ...
  // Sends either a keep-alive or topic update heartbeat to the subscriber in 'update' at
  // the closest possible time to the first member of 'update'.  If is_keepalive is true,
  // sends a keep-alive update, otherwise the set of pending topic updates is sent.
  void DoHeartbeat(bool is_keepalive, int thread_id,
  ...
  // their target topics.
  Status SendUpdate(Subscriber* subscriber, bool* heartbeat_skipped);
